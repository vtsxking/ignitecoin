...
export P_CI_DIR="${BASE_BUILD_DIR}/ignitecoin-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/ignitecoin-config.h > src/config/ignitecoin-config.h.tmp && mv src/config/ignitecoin-config.h.tmp src/config/ignitecoin-config.h'
fi
