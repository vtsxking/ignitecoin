...
static void SetupIgnitecoinUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " ignitecoin-util utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  ignitecoin-util [options] [commands]  Do stuff\n";
