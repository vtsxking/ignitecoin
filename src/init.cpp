 ...
 */
static const char* IGNITECOIN_PID_FILENAME = "ignitecoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", IGNITECOIN_PID_FILENAME)));
}
 ...
 * between ignitecoind, and ignitecoin-qt and non-server tools.
*/
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("ignitecoin-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", IGNITECOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", IGNITECOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ignitecoin/ignitecoin>";
    const std::string URL_WEBSITE = "<https://ignitecoincore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("ignitecoin-loadblk");
...
/** Sanity checks
 *  Ensure that Ignitecoin is running in a usable environment with all
...
{
    // Make sure only a single Ignitecoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", IGNITECOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if ignitecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ignitecoin is started while in a temporary directory.\n",
