...
const char * const IGNITECOIN_CONF_FILENAME = "ignitecoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ignitecoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ignitecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ignitecoin
    // Mac: ~/Library/Application Support/Ignitecoin
    // Unix: ~/.ignitecoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", IGNITECOIN_CONF_FILENAME);
