...
#if defined(NDEBUG)
# error "Ignitecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8d5057cd6c72bbc9e06725aa65a652abce45e8eb15d015097ab77411d1c06d29");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ignitecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ignitecoin Signed Message:\n";
...
{
    // Ignitecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ignitecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Ignitecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Ignitecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ignitecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ignitecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5017b5eaa641324dc94cffc45405c1a9e84f5bc6eabf44f058fdbe744ba66c06");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/Mar/2021 1 year and 1 day after Covid-19";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fa105dd670a6a866ca1dfdf9a58361b681894dd5430e0158e4a4b9cc7d87ed68d446280ea357746f9716d265bd15bc51ca0c39267edde48f77e76fdd9eec1bec") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615570340;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085820346;

        if (fTestNet)
        {
            block.nTime    = 1615570317;
            block.nNonce   = 385985786;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x969ac12f3e966d6b9b0cc588caa299fa75e7158d550dcd0b9ec18ed470670dac"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xb8, 0xdd }; // Ignitecoin: Random*
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IgnitecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IgnitecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IgnitecoinMiner(CWallet *pwallet)
{
    printf("IgnitecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ignitecoin-miner");
    ...
    try { loop {
        ...
        printf("Running IgnitecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IgnitecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IgnitecoinMiner, pwallet));
}
