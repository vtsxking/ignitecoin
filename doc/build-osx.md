...
This guide describes how to build ignitecoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Ignitecoin Core from source.
...
### 4. Clone Ignitecoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Ignitecoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `ignitecoind` or  `ignitecoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/ignitecoin/ignitecoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Ignitecoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/ignitecoin/ignitecoin/issues/7714).
...
You can deploy a `.dmg` containing the Ignitecoin Core application using `make deploy`.
...
## Building Ignitecoin Core

### 1. Configuration

There are many ways to configure Ignitecoin Core, here are a few common examples:
...
Run the following in your terminal to compile Ignitecoin Core:
...
## Running Ignitecoin Core

Ignitecoin Core should now be available at `./src/ignitecoind`.
If you compiled support for the GUI, it should be available at `./src/qt/ignitecoin-qt`.

The first time you run `ignitecoind` or `ignitecoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Ignitecoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Ignitecoin"

touch "/Users/${USER}/Library/Application Support/Ignitecoin/ignitecoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Ignitecoin/ignitecoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Ignitecoin/debug.log
```

## Other commands:

```shell
./src/ignitecoind -daemon      # Starts the ignitecoin daemon.
./src/ignitecoin-cli --help    # Outputs a list of command-line options.
./src/ignitecoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/ignitecoin-qt -server # Starts the ignitecoin-qt server mode, allows ignitecoin-cli control
```
