...
At a bare minimum, ignitecoind requires that the rpcpassword setting be set
when running as a daemon.  If the configuration file does not exist or this
setting is not set, ignitecoind will shut down promptly after startup.
...
PID file:            `/var/run/ignitecoind/ignitecoind.pid` (OpenRC and Upstart) or `/run/ignitecoind/ignitecoind.pid` (systemd)
Lock file:           `/var/lock/subsys/ignitecoind` (CentOS)  

The configuration file, PID directory (if applicable) and data directory
should all be owned by the ignitecoin user and group.  It is advised for security
reasons to make the configuration file and data directory only readable by the
ignitecoin user and group.  Access to ignitecoin-cli and other ignitecoind rpc clients
...
options specified in the init files (e.g. setting `IGNITECOIND_DATADIR` for
OpenRC).
...
setting the IGNITECOIND and FLAGS environment variables in the file
