# Fuzzing Ignitecoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Ignitecoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/ignitecoin/ignitecoin
$ cd ignitecoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/ignitecoin/ignitecoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/ignitecoin/ignitecoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/ignitecoin/ignitecoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/ignitecoin/ignitecoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`ignitecoin-core/qa-assets`](https://github.com/ignitecoin-core/qa-assets) repo.

To fuzz `process_message` using the [`ignitecoin-core/qa-assets`](https://github.com/ignitecoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/ignitecoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`ignitecoin-core/qa-assets`](https://github.com/ignitecoin-core/qa-assets) repo.

Every single pull request submitted against the Ignitecoin Core repo is automatically tested against all inputs in the [`ignitecoin-core/qa-assets`](https://github.com/ignitecoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Ignitecoin Core more robust.
...
with certain assembly code from Ignitecoin Core's code. See [developer notes on sanitizers](https://github.com/ignitecoin/ignitecoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Ignitecoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Ignitecoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Ignitecoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Ignitecoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Ignitecoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Ignitecoin
Core without having to write any custom fuzzing harness. The `ignitecoind` server
...
```sh
$ mkdir ignitecoin-honggfuzz-p2p/
$ cd ignitecoin-honggfuzz-p2p/
$ git clone https://github.com/ignitecoin/ignitecoin
$ cd ignitecoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/ignitecoind.cpp b/src/ignitecoind.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/ignitecoind.cpp
+++ b/src/ignitecoind.cpp
...
EOF
$ make -C src/ ignitecoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18483 -f inputs/ -- \
          src/ignitecoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18483 -logthreadnames \
                       -debug
```
