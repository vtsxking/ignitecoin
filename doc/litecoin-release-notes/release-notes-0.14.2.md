Ignitecoin Core version 0.14.2 is now available from:

  <https://download.ignitecoin.org/ignitecoin-0.14.2.0/>
...
Ignitecoin Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in Ignitecoin Core 0.10.4) it has been disabled by default.
...
Testnet3 has been deprecated and replaced with Testnet4. The server port has been changed to 7186 however the RPC port remains
the same (19332).

Testnet faucets can be located at:
- http://testnet.ignitecointools.com
- http://testnet.thrasher.io

Developers who require the new testnet blockchain paramaters can find them [here](https://github.com/ignitecoin-project/ignitecoin/blob/master/src/chainparams.cpp#L220).
...
Ignitecoin Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/ignitecoin-project/ignitecoin/blob/master/doc/zmq.md) for
...
The Alert System was disabled and deprecated in Ignitecoin Core 0.10.4 and removed in 0.13.2. 
...
here, originally written for Bitcoin but can also be used for Ignitecoin:
