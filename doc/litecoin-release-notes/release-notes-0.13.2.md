Ignitecoin Core version 0.13.2 is now available from:

  <https://download.ignitecoin.org/ignitecoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/ignitecoin-project/ignitecoin/issues>
...
updates will be released anymore. Without security updates, using a ignitecoin
...
Ignitecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Ignitecoin Core 0.13.2 also introduces new default policy limits on the length and
...
- Adding `dbcache=100` in ignitecoin.conf
...
ignitecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/ignitecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Ignitecoin Core code base has
...
- `ignitecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ignitecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Ignitecoin scripting system that in combination with
...
Ignitecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Ignitecoin Core.
...
The two phases are distinct in the Ignitecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Ignitecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ignitecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Ignitecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ignitecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ignitecoins being spent is exactly
...
- **Increased security for multisig:** Ignitecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Ignitecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Ignitecoin's current ledger (called the UTXO set),
  ...
  note that Ignitecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ignitecoin users to individually opt-in to almost any change in the Ignitecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ignitecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Ignitecoin
client to Ignitecoin Core 0.13.2 to signal for segwit support. When a
...
Since Ignitecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `ignitecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ignitecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
