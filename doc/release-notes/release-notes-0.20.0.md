...
Ignitecoin Core version 0.20.0 is now available from:

  <https://ignitecoincore.org/bin/ignitecoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Ignitecoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Ignitecoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Ignitecoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Ignitecoin Core. Ignitecoin Core logs debug messages
...
- OpenSSL is no longer used by Ignitecoin Core.  (#17265)

- BIP70 support has been fully removed from Ignitecoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/ignitecoin/ignitecoin/issues/16599),
  [PR #16702](https://github.com/ignitecoin/ignitecoin/pull/16702), and the
  `ignitecoind help` for more information.  This option is experimental and
...
- All custom settings configured when Ignitecoin Core starts are now
...
- The "Start Ignitecoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://ignitecoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Ignitecoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Ignitecoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/ignitecoin-core/HWI) for
  signing.  Future versions of Ignitecoin Core should support a GUI option
...
- Ignitecoin Core's automatically-generated source code documentation is
  now available at https://doxygen.ignitecoincore.org.  (#17596)
...
- The `ignitecoin-cli` utility used with the `-getinfo` parameter now
...
- The `ignitecoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `ignitecoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_ignitecoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_ignitecoin` binary (hebasto)
...
- #18702 Fix ASLR for ignitecoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate ignitecoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest ignitecoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_ignitecoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (ignitecoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone ignitecoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Ignitecoin Core files (hebasto)
...
- #17119 Fix broken ignitecoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from ignitecoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.ignitecoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to ignitecoin-wallet (hel-o)
...
- #18341 Replace remaining literal IGNC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On ignitecoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from ignitecoin-cli -getinfo (malevolent)
