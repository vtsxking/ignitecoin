...
Ignitecoin Core version 0.19.1 is now available from:

  <https://ignitecoincore.org/bin/ignitecoin-core-0.19.1/>
...
From Ignitecoin Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as Ignitecoin Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, Ignitecoin Core does not yet change appearance when
...
- #17488 fix "ignitecoind already running" warnings on macOS (fanquake)
