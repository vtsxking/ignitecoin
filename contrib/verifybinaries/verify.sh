#!/usr/bin/env bash
# Copyright (c) 2016 The Ignitecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   ignitecoincore.org and ignitecoin.org and compares them.
...
WORKINGDIR="/tmp/ignitecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ignitecoincore.org"
HOST2="https://ignitecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="ignitecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ignitecoin-',
...
if [ $? -ne 0 ]; then
   echo "ignitecoin.org failed to provide signature file, but ignitecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ignitecoin.org and ignitecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ignitecoin Core binary release signing key installed?"
   fi
